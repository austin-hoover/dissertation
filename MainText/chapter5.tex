...
One thing to note is that the zero-current tunes in the simulation were equal at $\nu_x = \nu_y = 6.18$. The measured tunes were also 6.18, but there may be some uncertainty in the measurement. The simulation was repeated as the horizontal tune was varied in steps of 0.005. At $\nu_x = 6.19$, there is not a large change from the original case. At $\nu_x = 6.2$, the cross-plane correlation in the beam is eliminated. In Fig.~\ref{fig:exp3_sim_nux6.195_nuy6.18}, the simulation is repeated with $\nu_x = 6.195$. 
%
\begin{figure}[!p]
    \centering
    \begin{subfigure}{0.85\textwidth}
...
\end{figure}
%
Let $t_{split}$ be the time at which the intrinsic emittances diverge from the apparent emittances. The effect of splitting the tunes is to push $t_{split}$ later in injection — in this case, near turn 200. The resulting emittance evolution more closely resembles the measurements in Fig.~\ref{fig:exp3_emittances}. 
