...
\begin{equation}
    \mathbf{x} = Re \left\{
        ...
        + \sqrt{2 J_1} \, \mathbf{v}_2 \, e^{-i\psi_2}
    \right\},
\end{equation}
%
...
The third-order resonance leads to a well-known triangular region of stability as the tune approaches 2/3. The bottom plot reveals fourth and fifth-order resonances only obtained from second-order perturbation theory.
...
\end{figure}
%
The instabilities violently affect the KV distribution, resulting in nearly three-fold emittance growth over a few periods, but their effect is less pronounced in the other distributions. Thus, it is assumed that higher-order coherent instabilities, while interesting, are not important in real beams with a large tune spread. It is argued that Landau damping is responsible.
...
\item
Something about more detailed tune resonance map of the ring?
...
\end{equation}
%
The coordinates are then transported to the injection point using the model transfer matrix. Repeating this for each BPM gives an estimated mean and standard deviation of the phase space coordinates at the injection point. Examples of individual and averaged BPM signals from the SNS are shown in Fig.~\ref{fig:bpm_avg} along with the damped-sinusoid fit. Additionally, a simulation of the $x$-$x'$ phase space is included to illustrate the decoherence of the minipulse.
%
\begin{figure}[!p]
    \centering
    \includegraphics[width=\textwidth]{Images/chapter1/bpm_avg.png}
    \caption{Turn-by-turn BPM signal — averaged over 50 pulses and fit with Eq.~\eqref{eq:damped_sinusoid}.}
    \label{fig:bpm_avg}
\end{figure}
%
