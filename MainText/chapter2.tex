...
\end{equation}
%
for all $s$, where $L$ is the period length and $\bm{\sigma}$ a vector of the ten unique elements of the covariance matrix. The problem of computing the matched envelope is as follows:
%
\begin{equation}
\begin{aligned}
    & \underset{\bm{\sigma}}{\text{minimize}}
    & & C(\bm{\sigma}) = \left\Vert{\mathbf{T} \bm{\sigma} - \bm{\sigma}}\right\Vert^2 \\
    & \text{subject to}
    & & |\bm{\Sigma}| = 0,
\end{aligned}
\end{equation}
%
where $\mathbf{T} = \mathbf{T}(\bm{\sigma})$ is a map connecting the initial and final envelope after tracking through one lattice period. The constraint that the covariance matrix must be singular comes from Eq.~\eqref{eq:mode_emittances2}. Additionally, we would like to hold the nonzero intrinsic emittance fixed so that a unique solution is found for a given lattice and beam perveance. An iterative approach is needed since space charge causes $\mathbf{T}$ to depend on $\bm{\sigma}$ in a potentially complicated way which is unknown before tracking the beam. 
...
Before presenting our solution, we highlight the space-charge-driven mismatch oscillations that are to be corrected. Fig.~\ref{fig:fodo_mismatch_withinlattice} and Fig.~\ref{fig:fodo_mismatch_tbt} show the within-lattice evolution and turn-by-turn evolution, respectively, of a beam that is matched to a lattice without space charge and then tracked with space charge. 
%
\begin{figure}[!p]
    \centering
...
\end{figure}
%
In Fig.~\ref{fig:fodo_mismatch_withinlattice}, the coupled nature of the space charge force is evident in the dynamic horizontal and vertical emittances. The turn-by-turn data in Fig.~\ref{fig:fodo_mismatch_tbt} shows two frequencies in the mismatch oscillations: a larger frequency near twice the zero-current tune corresponding to the breathing oscillation of the beam sizes and a smaller frequency corresponding to the emittance exchange. The initial emittances are split in this figure.
