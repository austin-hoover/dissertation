...
\end{equation}
%
The quadrupole field strengths are contained in the vector $\mathbf{g}$. The calculated and desired phase advances at WS24 are $\bm{\mu} = (\mu_x, \mu_y)$, and $\tilde{\bm{\mu}} = (\tilde{\mu}_x, \tilde{\mu}_y)$, respectively. The maximum calculated and allowed $\beta$ functions in the wire-scanner region are $\bm{\beta}_{max} = (\beta_{x_{max}}, \beta_{y_{max}})$ and $\tilde{\bm{\beta}}_{max} = (\tilde{\beta}_{x_{max}}, \tilde{\beta}_{y_{max}})$, respectively. $\Theta$ is the Heaviside step function. Finally, $\epsilon$ is a constant. We assume that the the beam is approximately matched to the lattice optics so that the calculated phase advances are close to the true phase advances.\footnote{As mentioned in Chapter \ref{chap-1}, phase space painting generally produces matched $x$-$x'$ and $y$-$y'$ distributions, especially after many accumulated turns. The opportunity for mismatch of the beam Twiss parameters with the lattice Twiss parameters comes at the ring extraction point — the entrance of the RTBT. Since the RTBT is not periodic, propagation of the Twiss parameters requires initial conditions. In all calculations of phase advances in the RTBT, we take the initial conditions to be those defined by the ring at the extraction point. Without space charge or nonlinear forces, there will be no mismatch. With space charge, the Twiss parameters in the ring will shift to new values: $\alpha$ generally remain the same while the $\beta$ increases. This change is small in production painting, but could be enhanced during our studies because of the lower beam energy and potentially smaller beam size. Nonetheless, the accuracy of the 2D emittance reconstruction error as a function of $\beta$ and $\alpha$ was studied in \cite{Prat2014}, who found that the method is accurate.}
